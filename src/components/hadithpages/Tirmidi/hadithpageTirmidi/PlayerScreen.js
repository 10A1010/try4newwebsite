...
const PlayerScreen = ({ chapterDetail }) => {
  const audiLink = (number) => number < 10 ? (
    "https://aatry1000.s3.amazonaws.com/Hadith/Tirmidi/0"+ number +"-Saxiix_Tirmidi.mp3") : (

      "https://aatry1000.s3.amazonaws.com/Hadith/Tirmidi/"+ number +"-Saxiix_Tirmidi.mp3")
      ...
      {chapterDetail !== null ? (
        <ul className='list-group text-end'>
          <div>
            ...
            ><br></br>
              <span className='fw-bold'>Mukhtasar Sunan Tirmidi</span>{" "}
            </li>
        ...
        <div className=' bg-red list-group-item home-body2'>
          <span className='fw-bold'><br></br>Mukhtasar Sunan Tirmidi </span>{" "}<br></br>
        </div>
      )}
    </div>
  );
