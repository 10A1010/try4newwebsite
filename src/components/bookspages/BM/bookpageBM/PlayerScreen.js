...
const PlayerScreen = ({ chapterDetail }) => {
  const audiLink = (number) => number < 10 ? (
    "https://aatry1000.s3.amazonaws.com/Books/BM/0"+ number +"+buluuqulmaraam.mp3") : (

      "https://aatry1000.s3.amazonaws.com/Books/BM/"+ number +"+buluuqulmaraam.mp3")
      ...
      {chapterDetail !== null ? (
        <ul className='list-group text-end'>
          <div>
            ...
            ><br></br>
              <span className='fw-bold'>Bulugul Miraam</span>{" "}
            </li>
        ...
        <div className=' bg-red list-group-item home-body2'>
          <span className='fw-bold'><br></br>Bulugul Miraam </span>{" "}<br></br>
        </div>
      )}
    </div>
  );
